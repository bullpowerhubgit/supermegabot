...
const { MultiAgentOrchestrator } = require('../core/multi-agent-orchestrator');
const { AuthSystem } = require('../core/auth-middleware');
const { PaymentIntegration } = require('../core/payment-integration');
const { TelegramWebhookV2 } = require('../api/webhook-telegram-v2');
...
// Initialize Telegram webhook V2
const telegramWebhook = new TelegramWebhookV2({ shopify: null }, orchestrator);

// Mount auth routes
app.use('/api/auth', authSystem.getRouter());

// Mount payment routes
app.use('/api/billing', paymentSystem.getRouter());

// Mount Telegram webhook
app.post('/webhooks/telegram', (req, res) => telegramWebhook.handleRequest(req, res));

// Mount multi-agent orchestrator routes
app.use('/multi-agent', multiAgentOrchestrator.getRouter());

// Mount legacy orchestrator routes
...
app.get('/api/health', async (req, res) => {
  ...
  // Critical services must be available for "ok" status
  ...
  const statusCode = criticalOk ? 200 : (allOk ? 206 : 503);
