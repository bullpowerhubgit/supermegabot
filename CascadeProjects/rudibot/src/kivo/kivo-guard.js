/**
 * KIVO Guard (Embedded) — Pairing, Approval, Roles, Secure Release
 * Embedded security layer for KIVO in Rudibot
 */
...
const SENSITIVE_ACTIONS = [
  { pattern: 'finance.subscription', level: 2, reason: 'Subscription cancellation is irreversible' },
  { pattern: 'finance.tax', level: 2, reason: 'Tax export involves legal liability' },
  { pattern: 'security.scan', level: 1, reason: 'Security scan may trigger alerts' },
  { pattern: 'system.restart', level: 2, reason: 'Restart interrupts services' },
  { pattern: 'system.deploy', level: 3, reason: 'Deployment affects production' },
];

class EmbeddedKivoGuard {
  constructor(options = {}) {
    this.role = options.role || process.env.KIVO_ROLE || 'user';
    this.roleConfig = ROLES[this.role] || ROLES.user;
    this.sessionApprovals = new Set();
    this.auditLog = [];
    this.allowedUserIds = options.allowedUserIds ? new Set(options.allowedUserIds) : null;
  }

  // ── User Authorization ─────────────────────────────────────
  isUserAuthorized(userId) {
    if (!this.allowedUserIds) return true; // No restriction
  ...
  createApprovalKeyboard(actionId) {
    return {
      inline_keyboard: [
        [
  ...
  formatApprovalMessage(reason, action) {
    return `⚠️ *APPROVAL REQUIRED*\n\n${reason}\n\nAction: ${action}\n\nChoose below to continue or cancel.`;
  }
  ...
  getStatus() {
    return {
      ...
      authorizedUsers: this.allowedUserIds?.size || 'unrestricted'
    };
  }
  ...
  clearSessionApprovals() {
    ...
    this.log('session_cleared', { timestamp: new Date().toISOString() });
  }
}

module.exports = { EmbeddedKivoGuard, ROLES, APPROVAL_LEVELS, SENSITIVE_ACTIONS };
