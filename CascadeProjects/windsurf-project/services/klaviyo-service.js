/**
 * Centralized Klaviyo Service with Retry Logic
 * Handles all Klaviyo API operations with proper retry and queue management
 */
...
class KlaviyoService extends EventEmitter {
  constructor(config = {}) {
    super();
    this.apiKey = config.apiKey || process.env.KLAVIYO_API_KEY;
   ...
   * Add to list
   */
  async addToList(listId, profileIds, options = {}) {
    const requestData = {
      method: 'POST',
    ...
    try {
      const response = await this.client.post(`/lists/${listId}/relationships/profiles/`, requestData.data);
      return response.data;
    } catch (error) {
      this.emit('error', { operation: 'addToList', error: error.message });
      throw error;
    }
  }
  ...
  async getCampaign(campaignId) {
    try {
      ...
      this.emit('error', { operation: 'getCampaign', error: error.message });
      throw error;
    }
  }
  ...
  async sendCampaign(campaignId, options = {}) {
    ...
    try {
      const response = await this.client.post(`/campaigns/${campaignId}/send/`, requestData.data);
      return response.data;
    } catch (error) {
      this.emit('error', { operation: 'sendCampaign', error: error.message });
      throw error;
    }
  }
