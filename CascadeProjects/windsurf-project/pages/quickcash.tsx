...
const CostDashboard = ({ stats, darkMode: dm }: { stats: any, darkMode: boolean }) => {
  if (!stats) return (
...
const QuickCashSystem = () => {
  ...
  const [runningTools, setRunningTools] = useState(new Set<number>());
  ...
  const [apiKey, setApiKey] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('quickcash_api_key') || '';
    }
    return '';
  });
  const [backendUrl, setBackendUrl] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('quickcash_backend_url') || 'http://localhost:3001';
    }
  ...
  const addOutput = (toolId: number, message: string, type: string) =>
    setToolOutput(prev => [...prev, { toolId, message, type: type || 'info', timestamp: new Date().toLocaleTimeString() }]);

  const recordStats = (toolId: number, inp: number, out: number) => {
  ...
  const runTool = async (tool: any) => {
    ...
    const missing = tool.fields.filter((f: any) => !config[f.name]);
    if (missing.length) { 
      addOutput(tool.id, 'Bitte ausfüllen: ' + missing.map((f: any) => f.label).join(', '), 'error'); 
      return; 
    }
  ...
  const totalSession = Object.values(sessionStats).reduce((acc: any, s: any) => ({
        ...
        <aside className={dm ? 'bg-gray-800 border-r border-gray-700 w-52 p-4 flex flex-col gap-2' : 'bg-white border-r border-gray-200 w-52 p-4 flex flex-col gap-2'}>
          ...
          {Object.keys(sessionStats).length > 0 && (
            <div className={`mt-2 p-3 rounded-lg border ${dm ? 'bg-purple-900/20 border-purple-500/30' : 'bg-purple-50 border-purple-200'}`}>
              ...
              {Object.entries(sessionStats).map(([tid, s]: any) => {
                const t = quickCashTools.find((x: any) => x.id === parseInt(tid));
                return (
          ...
          {activeTab === 'downloads' && (
            <div className="space-y-5">
              ...
              {Object.keys(generatedAssets).length === 0 ? (
                ...
                const tool = quickCashTools.find((t: any) => t.id === parseInt(toolId));
                  ...
                  <div key={toolId} className={`${card} rounded-xl p-5`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {Object.entries(assets).map(([name, content]) => (
                        <button key={name} onClick={() => downloadFile(name, content as string)}
