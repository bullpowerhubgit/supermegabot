...
async function fetchEtsyTrends(keyword: string): Promise<{ products: Array<{ title: string; price: number; views: number }>; trendScore: number }> {
  try {
    ...
    if (!config.etsy?.apiKey || config.etsy.apiKey === '' || config.etsy.apiKey === 'txbp26vgg2wb0otqt4v9fvbj') {
      console.warn('Etsy API Key nicht konfiguriert oder ist Beispiel-Key, nutze Mock-Daten')
      return {
        products: [
          { title: `${keyword} Inspired T-Shirt`, price: 24.99, views: 1234 },
          { title: `Vintage ${keyword} Design`, price: 32.50, views: 856 },
          { title: `${keyword} Premium Hoodie`, price: 45.00, views: 2103 }
        ],
        trendScore: 75
      }
    }
    ...
    if (!res.ok) {
      console.warn('Etsy API request failed:', res.status)
    ...
    return { 
      products: [
        { title: `${keyword} Basic Tee`, price: 22.99, views: 567 },
        { title: `${keyword} Art Print`, price: 28.00, views: 445 }
      ], 
      trendScore: 45 
    }
  }
...
// GCP Vertex AI API — Advanced AI analysis
async function callVertexAI(prompt: string): Promise<{ response: string; model: string }> {
  try {
    const configRes = await fetch('./api-config.json')
    const config: ApiConfig = await configRes.json()
    
    if (!config.gcp?.projectId) {
      console.warn('GCP Config nicht gefunden, nutze Claude Fallback')
      return { response: 'GCP Vertex AI nicht konfiguriert', model: 'claude-fallback' }
    }
    
    // Vertex AI API call (requires GCP auth in production)
    const vertexEndpoint = `https://${config.gcp.projectId}-aiplatform.googleapis.com/v1/projects/${config.gcp.projectId}/locations/us-central1/publishers/google/models/gemini-pro:generateContent`
    
    const res = await fetch(vertexEndpoint, {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${config.gcp.projectId}` // Simplified auth
      },
      body: JSON.stringify({
    ...
    if (!res.ok) {
      console.warn('Vertex AI failed, nutze Claude Fallback')
      return { response: 'Vertex AI nicht verfügbar', model: 'claude-fallback' }
    }
    
    const data = await res.json()
    return { 
      response: data.candidates?.[0]?.content?.parts?.[0]?.text || 'Keine Antwort',
      model: 'vertex-ai-gemini-pro'
    }
  } catch (e) {
    console.warn('Vertex AI Fehler:', e)
    return { response: 'Vertex AI Fehler', model: 'claude-fallback' }
  }
}
...
export default function AutoShopSuite() {
  ...
  async function runPricingAI() {
    ...
    try {
      ...
      setPAdv(r.text); setCntPrice(n=>n+1)
      addHistory('Pricing', `EK:${pBuy}€ VK:${pVk}`, r.text)
  ...
  async function runDesignsLib() {
    ...
    try {
      ...
      setDlOut(r.text)
      const newEntry = { id:Date.now(), theme:dlTheme, type:dlType, style:dlStyle, prompts:r.text, ts:new Date().toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'}) }
      setDesignLib(prev=>[...prev, newEntry])
      setDlSelected(newEntry.id)
      setCntDesign(n=>n+5)
      addHistory('Design Library', `${dlTheme} (${dlType})`, r.text)
