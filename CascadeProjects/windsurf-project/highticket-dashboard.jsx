  ...
  const callClaude = async (systemPrompt, userMsg) => {
    ...
    try {
      ...
      if (configResponse.ok) {
        ...
        if (config.anthropic) {
          apiKeyToUse = config.anthropic.apiKey || apiKeyToUse;
          model = config.anthropic.model || model;
          baseUrl = config.anthropic.baseUrl || baseUrl;
          version = config.anthropic.version || version;
        }
      }
    } catch (e) {
      console.warn('API config nicht gefunden, nutze Standardwerte');
    }

    if (!apiKeyToUse || apiKeyToUse === 'DEIN_ANTHROPIC_API_KEY_HIER' || apiKeyToUse === 'sk-ant-api03-DEIN_API_KEY_HIER') {
      throw new Error('Bitte echten Anthropic API-Key in api-config.json eintragen!');
    }
...
Antworte NUR mit diesem JSON (kein Markdown, kein Text drumherum):
{
  ...
  "risikoLevel": "niedrig|mittel|hoch"
    ...
    try {
      ...
      setPricingResult({ error: "Konnte nicht parsen: " + raw });
    }
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 72 }}>
          ...
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            <input
              ...
              style={{ background: "#0a0a0a", border: "1px solid #2a1a0a", borderRadius: "4px", color: "#c9a84c", padding: "6px 10px", fontSize: "11px", width: "200px", outline: "none", fontFamily: "monospace" }}
            />
            {[
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
              <div style={{ background: "#111008", border: "1px solid #2a2010", borderRadius: 12, padding: 28 }}>
                ...
                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontSize: 12, color: "#4a3f2a", marginBottom: 8 }}>Kontaktkanal</div>
                ...
                {funnelScript ? (
                  <div style={{ background: "#111008", border: "1px solid #c9a84c30", borderRadius: 12, padding: 28 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                      <div style={{ fontSize: 12, color: "#c9a84c", letterSpacing: "0.3em", textTransform: "uppercase" }}>✦ VERKAUFS-SKRIPT</div>
                      <button onClick={() => navigator.clipboard.writeText(funnelScript)} style={{ padding: "6px 14px", background: "transparent", border: "1px solid #2a2010", borderRadius: 6, color: "#6b5a3a", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>KOPIEREN</button>
                    </div>
                    <div style={{ fontSize: 14, color: "#b8a888", lineHeight: 1.9, whiteSpace: "pre-wrap", fontFamily: "inherit" }}>{funnelScript}</div>
                  </div>
                ) : (
                  <div style={{ background: "#111008", border: "1px solid #2a2010", borderRadius: 12, padding: 28, display: "flex", alignItems: "center", justifyContent: "center", minHeight: 300 }}>
                    <div style={{ textAlign: "center", color: "#2a2010" }}>
