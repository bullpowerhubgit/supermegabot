...
// TYPES
// ══════════════════════════════════════════════════════════════════════════════
type NavTab = 'dash' | 'pod' | 'drop' | 'designs' | 'workflows' | 'pricing' | 'settings' | 'upload'
...
// ══════════════════════════════════════════════════════════════════════════════
// VERTEX AI API  — calls GCP Cloud Function (Gemini 1.5 Pro) with Fallback
// Project: gen-lang-client-0895465231
// Fallback: My-Shop Backend Claude Proxy (localhost:4001)
// ══════════════════════════════════════════════════════════════════════════════
async function callClaude(prompt: string, proxyUrl = 'https://europe-west1-gen-lang-client-0895465231.cloudfunctions.net/vertexAIProxy', model = 'gemini-1.5-pro'): Promise<string> {
  // Try GCP proxy first
  try {
    ...
    console.warn('GCP Proxy failed, falling back to My-Shop Backend:', gcpError)
    // Fallback to My-Shop Backend Claude proxy
    try {
      const res = await fetch('http://localhost:4001/api/claude', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt, model: 'claude-sonnet-4-20250514', maxTokens: 1200 }),
      })
      const d = await res.json()
      if (d.error) throw new Error(`Fallback API-Fehler: ${d.error}`)
      return d.text ?? ''
    } catch (fallbackError) {
      throw new Error(`Both proxies failed. GCP: ${(gcpError as Error).message}. Fallback: ${(fallbackError as Error).message}`)
    }
  }
}
...
const DEFAULT_SETTINGS: Settings = {
  proxyUrl: 'https://europe-west1-gen-lang-client-0895465231.cloudfunctions.net/vertexAIProxy',
...
export default function AutoShopSuite() {
  ...
  // ── Upload ───────────────────────────────────────────────────────────────────
  const [uploadPlatform, setUploadPlatform] = useState('Etsy')
  ...
  async function runUpload() {
    if(!uploadProduct.trim() || !uploadPrice.trim()) return alert('Produktname und Preis eingeben!')
    setUploadLoading(true); setUploadResult('')
    try {
      const r = await callClaude(
        `E-Commerce Upload-Experte für ${uploadPlatform}. Erstelle vollständigen Upload-Text für:\n\nProdukt: ${uploadProduct}\nPreis: ${uploadPrice}€\nBild: ${uploadImage || 'Nicht angegeben'}\n\n## LISTING-TITEL\nSEO-optimiert, max 140 Zeichen\n\n## BESCHREIBUNG\nVerkaufstark, emotional, Features, Vorteile\n\n## TAGS\n12-15 relevante Tags, kommasepariert\n\n## PREIS-STRATEGIE\nEmpfehlung basierend auf Preis\n\nDeutsch, professionell.`,
        settings.proxyUrl, settings.model
      )
      setUploadResult(r)
      addHistory('Produkt-Upload', `${uploadPlatform}: ${uploadProduct}`, r)
    } catch(e) {
      setUploadResult(`❌ Fehler: ${(e as Error).message}`)
    }
  ...
  const navItems: { id:NavTab; icon:string; label:string; color:string }[] = [
    ...
    { id:'upload',    icon:'📤', label:'Produkt-Upload',  color:C.green  },
    ...
    <div style={{ background:C.bg, color:C.text, fontFamily:"'Syne',sans-serif", height:'100vh', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <header style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 24px', height:52, background:C.bg2, borderBottom:`1px solid ${C.b1}`, flexShrink:0, position:'relative', zIndex:10 }}>
        ...
        <div style={{ display:'flex', gap:14, alignItems:'center' }}>
          <span style={{ fontSize:10, ...MN, color:C.muted }}>{settings.model.replace('claude-','').replace('-20250514','')}</span>
      ...
      <div style={{ flex:1, display:'flex', overflow:'hidden', position:'relative', zIndex:1 }}>
        ...
        <main style={{ flex:1, overflowY:'auto', padding:'20px 24px' }}>
          ...
          {nav==='upload' && (
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
                <h2 style={{ fontSize:18, fontWeight:800, margin:0 }}>📤 Produkt-Upload</h2>
                <Badge color={C.green} bg="rgba(0,230,118,.1)" border="rgba(0,230,118,.3)">POD & DS</Badge>
              </div>

              <ToolDash stats={[
                {icon:'📤',val:uploadResult?'✓':'—', label:'Uploads bereit',    color:C.green},
                {icon:'🎨',val:uploadPlatform,       label:'Plattform',        color:C.amber},
                {icon:'💰',val:uploadPrice||'—',      label:'Preis',            color:C.cyan},
                {icon:'⚡',val:uploadLoading?'Läuft':'Bereit',label:'Status',       color:uploadLoading?C.green:C.muted},
              ]}/>

              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                {/* Upload Form */}
                <div style={CARD}>
                  <CardTitle dot={C.green}>PRODUKT-UPLOAD GENERATOR</CardTitle>
                  <FG label="Plattform">
                    <select style={SI} value={uploadPlatform} onChange={e=>setUploadPlatform(e.target.value)}>
                      {['Etsy','Printful','Printify','Shopify','Amazon Merch','Redbubble'].map(p=><option key={p}>{p}</option>)}
                    </select>
                  </FG>
                  <FG label="Produktname"><input style={SI} value={uploadProduct} onChange={e=>setUploadProduct(e.target.value)} placeholder="z.B. Vintage Cat Mom T-Shirt"/></FG>
                  <FG label="Preis (€)"><input style={SI} type="number" value={uploadPrice} onChange={e=>setUploadPrice(e.target.value)} placeholder="24.99"/></FG>
                  <FG label="Bild-URL (optional)"><input style={SI} value={uploadImage} onChange={e=>setUploadImage(e.target.value)} placeholder="https://example.com/image.jpg"/></FG>
                  <div style={{ display:'flex', gap:8 }}>
                    <Btn color={C.green} onClick={runUpload} disabled={uploadLoading} full>{uploadLoading?'⏳ Generiere…':'📤 Upload-Text erstellen'}</Btn>
                  </div>
                </div>

                {/* Output */}
                <div style={CARD}>
                  <CardTitle dot={C.cyan}>GENERIERTER UPLOAD-TEXT</CardTitle>
                  <Output text={uploadResult} loading={uploadLoading} minH={200}/>
                  {uploadResult && (
                    <div style={{ marginTop:10, display:'flex', gap:6 }}>
                      <GBtn onClick={()=>cp(uploadResult)}>📋 Kopieren</GBtn>
                      <GBtn onClick={()=>exportTxt(uploadResult,'UploadText')}>💾 TXT exportieren</GBtn>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                ...
                <div style={CARD}>
                  ...
                  <FG label="Proxy URL (GCP Cloud Function)">
                    <input style={SI} value={settingsDraft.proxyUrl} onChange={e=>setSettingsDraft(d=>({...d,proxyUrl:e.target.value}))} placeholder="https://europe-west1-gen-lang-client-0895465231.cloudfunctions.net/vertexAIProxy"/>
                  </FG>
                ...
                <div style={CARD}>
                  ...
                  <div style={{ marginTop:10, ...CARD, padding:10, border:`1px solid ${C.b2}`, background:C.bg }}>
                    ...
                    <div style={{ fontSize:11, ...MN, color:C.text }}>
                      <div>Model: <span style={{ color:C.cyan }}>{settings.model.replace('claude-','').replace('-20250514','')}</span></div>
