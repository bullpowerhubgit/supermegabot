...
const QuickCashSystem = () => {
  ...
  const callClaude = async (prompt) => {
    if (!apiKey) {
      ...
      const mockResponses = {
        'Create Fiverr gig': `PROFESSIONELLES FIVERR GIG

**Titel:** Expert ${prompt.includes('Logo') ? 'Logo Design' : prompt.includes('Content') ? 'Content Writing' : 'AI Service'} | Express Delivery | Premium Quality

**Beschreibung:**
Professionelle ${prompt.includes('Logo') ? 'Logo Design' : prompt.includes('Content') ? 'Content Writing' : 'AI Services'} mit schneller Lieferung. 5+ Jahre Erfahrung, über 1.000 zufriedene Kunden.

**Pakete:**
• Basic: $25 - Lieferung in 48 Stunden
• Standard: $50 - Lieferung in 24 Stunden  
• Premium: $100 - Lieferung in 12 Stunden + 2 Revisionen

**Tags:** logo design, branding, graphic design, professional, fast delivery

**FAQ:**
Q: Wie viele Revisionen? A: 1 bei Basic, 2 bei Standard/Premium
Q: Dateiformate? A: PNG, JPG, SVG, AI, PSD
Q: Kommerzielle Nutzung? A: Ja, bei allen Paketen inklusive`,

        'Lead gen system': `LEAD-GENERATION SYSTEM

**Zielkriterien:**
• Einkommen > $80.000/Jahr
• Entscheidungsträger (Owner, CEO, Director)
• Local Business (5-50 Mitarbeiter)
• Digital aktiv (Website, Social Media)

**Quellen:**
1. Google Maps "near me" Suche
2. LinkedIn Sales Navigator
3. lokale Branchenverbände
4. Instagram Hashtag Recherche

**Cold Email Sequence:**
1. Tag 1: Problem + Lösung
2. Tag 3: Social Proof + Case Study
3. Tag 5: Limited Time Offer

**Preisstrategie:**
• Starter: $25/Lead (5 Leads)
• Business: $20/Lead (10 Leads)
• Enterprise: $15/Lead (20+ Leads)`,

        'Upwork system': `UPWORK AUTOMATION SYSTEM

**Profile Optimierung:**
• Professional Headshot + Background
• Keyword-reiche Titel/Beschreibung
• Spezialisierte Portfolio-Items
• 95%+ Job Success Rate

**Automatisierte Bewerbungen:**
1. Job Alerts mit Keywords filtern
2. AI-generierte Cover Letters
3. Schnelle Antwort (<30 Minuten)
4. Follow-up nach 24 Stunden

**AI Delivery Shortcuts:**
• Claude für Content Creation
• ChatGPT für Research
• Midjourney für Visuals
• Grammarly für Quality Check

**Preis-Strategie:**
• Entry Level: $20-25/Stunde
• Mid Level: $25-35/Stunde  
• Expert Level: $35-50/Stunde`
      };
      
      // Find matching mock response
      let mockText = mockResponses['Create Fiverr gig']; // default
      for (const [key, value] of Object.entries(mockResponses)) {
        if (prompt.toLowerCase().includes(key.toLowerCase().substring(0, 10))) {
          mockText = value;
          break;
        }
      }
      
      return { text: mockText, inputTokens: 5000, outputTokens: 1500 };
    }
    
    // Try to load API config for additional settings
    let apiConfig = { model: 'claude-sonnet-4-20250514', maxTokens: 1000 };
    try {
      const configResponse = await fetch('./api-config.json');
      if (configResponse.ok) {
        const config = await configResponse.json();
        if (config.anthropic) {
          apiConfig.model = config.anthropic.model || apiConfig.model;
          apiConfig.maxTokens = config.anthropic.maxTokens || apiConfig.maxTokens;
        }
      }
    } catch (e) {
