...
def get_log_stats():
    import os
    log = Path(os.getenv("SUPERMEGABOT_DIR", str(Path(__file__).parent.parent))) / "logs" / "dashboard.log"
...
# Save
import os as _os
out = Path(_os.getenv("SUPERMEGABOT_DIR", str(Path(__file__).parent.parent))) / "dashboard" / "mega_monitor.html"
fig.write_html(
    str(out),
